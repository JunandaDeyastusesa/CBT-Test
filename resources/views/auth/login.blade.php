...
<body class="font-poppins text-[#0A090B]">
    <section id="signup" class="flex w-full min-h-[100%]">
        <nav class="flex items-center text-center px-[50px] pt-[30px] w-full absolute top-0">
                ...
                <ul class="flex items-center gap-[30px]">
                    <li class="h-[52px] flex items-center">
                        <a href="{{ route('register') }}"
                            class="font-semibold text-white p-[14px_30px] bg-[#0A090B] rounded-full text-center">Daftar
                            Akun</a>
                    </li>
                </ul>
            </div>
        ...
        <div class="left-side min-h-screen flex flex-col w-full pb-[30px] pt-[82px]">
            <div class="h-full w-full flex items-center justify-center">
                <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-[30px] w-[90%] shrink-0">
                    @csrf
                    ...
                    <a href="{{ route('register') }}"
                        class="text-gray-500 rounded-full text-center hover:text-black">Belum Punya Akun? <span class="font-semibold">Daftar Disini</span></a>
                </form>
            </div>
        </div>

        <div
            class="right-side hidden md:block min-h-screen flex flex-col w-[650px] shrink-0 pb-[30px] pt-[82px] bg-[#5236f1]">
